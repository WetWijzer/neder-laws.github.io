...
export async function answerWithGraphRag(
  ...
  if (evidence.sections.length === 0) {
    return {
      ...
      answer:
        'I could not find a relevant Dutch law article in the local corpus for that question.',
      evidence,
      logicEvidence,
...
function shouldSkipLocalModelForBrowserTest() {
  ...
  return window.localStorage.getItem('WETWIJZER_DISABLE_LOCAL_LLM') === 'true';
}
...
Excerpt: ${truncateText(cleanCorpusExcerpt(excerptSource), excerptLimit)}${logicBlock}`;
  ...
  const prompt = `You answer questions about Dutch law using only the evidence below.
This is legal information, not legal advice.
${sectionScoped ? 'The first evidence item is the selected article the user is reading. Use it as the primary context before relying on related laws.' : 'The evidence was retrieved from the local corpus for the user question.'}
...
function buildEvidenceSummary(sections: SearchResult[]) {
  ...
  return `The strongest local corpus matches are:\n\n${lead}\n\nReview the cited Dutch law articles and verify official text on wetten.overheid.nl before relying on this information.`;
}

function cleanCorpusExcerpt(excerpt: string) {
  return excerpt
    .replace(/\s+/g, ' ')
    .replace(/(^|\.\.\.|\s)Label:\s*(City code section|Dutch law article)\s*/gi, '$1')
