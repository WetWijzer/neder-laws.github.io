...
def extract_sections(corpus_root):
    rows = read_rows(corpus_root / "canonical" / f"{DATASET_STEM}.parquet", SECTION_FIELDS)
    sections = []
    section_index = {}
...
def extract_embeddings(corpus_root):
    rows = read_rows(
        corpus_root / "canonical" / f"{DATASET_STEM}_embeddings.parquet",
...
def extract_bm25(corpus_root):
    rows = read_rows(
        corpus_root / "canonical" / f"{DATASET_STEM}_bm25.parquet",
        [
            "id",
            "document_id",
            "title",
...
def extract_graph(corpus_root):
    entity_rows = read_rows(
        corpus_root / "canonical" / f"{DATASET_STEM}_knowledge_graph_entities.parquet",
        ["id", "type", "label", "properties_json"],
    )
    relationship_rows = read_rows(
        corpus_root / "canonical" / f"{DATASET_STEM}_knowledge_graph_relationships.parquet",
...
def extract_logic_summary(corpus_root):
    candidates = [
        ...
        / "logic_proofs_codex_spark_groth16"
            / f"{DATASET_STEM}_logic_proof_artifacts.parquet",
        corpus_root
        / "logic_proofs_codex_spark"
        / f"{DATASET_STEM}_logic_proof_artifacts.parquet",
        corpus_root / "logic_proofs" / f"{DATASET_STEM}_logic_proof_artifacts.parquet",
    ]
...
def main():
    if len(sys.argv) != 2:
        raise SystemExit("Usage: extract-netherlands-corpus.py <corpus-root>")
