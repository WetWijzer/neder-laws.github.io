...
Last updated: 2026-05-05 08:11:12 UTC
...
- [!] `Task checkbox-476: Manual unblock: port 'logic/CEC/native/event_calculus.py' by adding event, fluent, happens, holds, initiates, terminates, and timeline query helpers with parity tests.` - latest daemon round failed validation or preflight
...
- Summary: Added Python-style CEC event calculus helper exports for event/fluent construction, predicate queries, fluent snapshots, and timelines with parity tests.
- Impact: The browser-native TypeScript CEC module now exposes direct event, fluent, happens, holds, initiates, terminates, fluentsAt, and timeline helpers over the native Event Calculus engine, and the Jest suite exercises those helpers without Python, server, filesystem, or RPC fallbacks.
- Accepted changed files: `docs/IPFS_DATASETS_LOGIC_TYPESCRIPT_PORT_PLAN.md`, `src/lib/logic/cec/eventCalculus.test.ts`, `src/lib/logic/cec/eventCalculus.ts`
