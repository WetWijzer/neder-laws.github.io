...
export interface CorpusSection {
  ...
  title_number: string;
  jsonld: string;
  law_status?: 'current' | 'historical' | 'repealed' | 'superseded' | 'unknown';
...
let manifestPromise: Promise<WetWijzerCorpusManifest> | null = null;
...
export async function fetchCorpusJson<T>(relativePath: string): Promise<T> {
  const manifest = await loadWetWijzerCorpusManifest();
...
export async function getSection(ipfs_cid: string): Promise<CorpusSection | null> {
  const { sectionByCid } = await loadWetWijzerCorpus();
